...
class SnackService {
  ...
  Future<Map<String, dynamic>?> _lookupByBarcode(String barcode) async {
    try {
      final uri = Uri.parse(
        '$_base/api/v0/product/$barcode.json?fields=product_name,product_name_en,image_front_url,image_url,categories',
      );
  ...
  String _servingSuggestionFor(String name, String offCategory) {
    ...
    // Cereals & breakfast
    if (n.contains('cornflakes') || n.contains('corn flakes'))
      return 'Pour into a bowl, add cold milk — done in 1 min';
    if (n.contains('cereal') ||
        n.contains('granola') ||
        n.contains('muesli') ||
        c.contains('cereal'))
      return 'Add milk or yogurt for a quick no-cook breakfast';
    if (n.contains('oat') || n.contains('oatmeal') || n.contains('quaker'))
      return 'Add hot water or milk and stir — ready in 3 min';
    if (n.contains('milo') ||
        n.contains('ovaltine') ||
        n.contains('bournvita') ||
        n.contains('horlicks'))
      return 'Mix with hot or cold milk for an instant energy drink';

    // Bread & spreads
    if (n.contains('bread') || c.contains('bread') || c.contains('bakery'))
      return 'Toast and spread with butter, jam, or peanut butter';
    if (n.contains('peanut butter') ||
        n.contains('jam') ||
        n.contains('jelly') ||
        n.contains('spread'))
      return 'Spread on bread or crackers for a quick bite';
    if (n.contains('margarine') || n.contains('butter'))
      return 'Spread on toast or warm bread — ready instantly';

    // Eggs
    if (n.contains('egg')) return 'Boil, fry, or microwave in under 5 min';

    // Instant noodles
    if (n.contains('indomie') ||
        n.contains('instant noodle') ||
        n.contains('cup noodle') ||
        n.contains('maggi noodle'))
      return 'Boil for 3 min, add seasoning — fastest hot meal ever';
    if (n.contains('noodle') || c.contains('noodle'))
      return 'Quick 5-min meal: boil and season to taste';

    // Dairy
    if (n.contains('yogurt') || n.contains('yoghurt') || c.contains('yogurt'))
      return 'Top with honey or granola for a quick snack';
    if (n.contains('milk') || c.contains('dairy'))
      return 'Drink cold or mix with cereal';
    if (n.contains('cheese'))
      return 'Slice onto crackers or bread — no cooking needed';

    // Chips & crisps
    if (n.contains('pringles') ||
        n.contains('chips') ||
        n.contains('crisp') ||
        c.contains('chips'))
    ...
    if (n.contains('chocolate') || c.contains('chocolate'))
      return 'Eat as-is or melt over ice cream';
    ...
    if (n.contains('juice') ||
        ...
        c.contains('beverage') ||
        c.contains('juice'))
      return 'Serve chilled — no prep needed';

    // Nuts
    if (n.contains('cashew') ||
        n.contains('almond') ||
        n.contains('groundnut') ||
        n.contains('peanut') ||
        c.contains('nut'))
      return 'Grab a handful — great protein snack on the go';
    ...
    if (n.contains('sardine') ||
        ...
        c.contains('fish'))
      return 'Serve on crackers or toast — open and eat';

    // African quick bites
    if (n.contains('garri') || n.contains('gari'))
      return 'Soak in cold water with sugar and groundnuts for a quick snack';
    if (n.contains('plantain chip') || n.contains('chin chin'))
      return 'Ready to eat straight from the pack';
    ...
    if (n.contains('banana') ||
        ...
        n.contains('orange') ||
        n.contains('mango'))
      return 'Rinse and enjoy as-is — nature\'s fastest snack';

    // Popcorn & corn
    if (n.contains('popcorn') || n.contains('corn'))
  ...
  String _emojiFor(String name, String offCategory) {
    ...
    if (n.contains('milo') || n.contains('ovaltine') || n.contains('bournvita'))
      return '☕';
    if (n.contains('bread') || c.contains('bread') || c.contains('bakery'))
      return '🍞';
    if (n.contains('peanut butter')) return '🥜';
    if (n.contains('jam') || n.contains('jelly') || n.contains('honey'))
      return '🍯';
    ...
    if (n.contains('biscuit') ||
        ...
        c.contains('biscuit'))
      return '🍪';
    if (n.contains('juice') || n.contains('drink') || c.contains('juice'))
      return '🥤';
    if (n.contains('cashew') ||
        n.contains('almond') ||
        n.contains('peanut') ||
